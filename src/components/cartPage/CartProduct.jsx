...
export const CardProduct = ({ item }) => {
  ...
  const { removeProductToWishList, handlewishlistCheck, addProductToWishList } =
    WishListState();
      ...
      <div className="section-one">
        ...
        <div className="product-info">
          ...
          <div className="price-section action-button">
            <button
              onClick={() => CartQUantHandler({ item, type: "increment" })}
            >
              {" "}
              +
            </button>

            <button>{item?.qty && item?.qty >= 1 ? item?.qty : 1}</button>
            <button
              onClick={() => CartQUantHandler({ item, type: "decrement" })}
            >
              {" "}
              -
            </button>
